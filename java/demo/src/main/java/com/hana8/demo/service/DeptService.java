...
public class DeptService {
	...
	public DeptDTO getDept(Integer id) {
		Dept dept = repository.findById(id)
			.orElseThrow(() -> new IllegalArgumentException("Dept #%d not found".formatted(id)));
	...
	public DeptDTO registDept(DeptDTO dto) {
		...
		List<Member> members = dto.getDeptMembers().stream().map(m ->
			memberRepository.findById(m.getId()).orElseThrow(() -> new IllegalArgumentException("member is not found"))
		).toList();
	...
	public DeptDTO editDept(Integer id,DeptDTO dto) {
