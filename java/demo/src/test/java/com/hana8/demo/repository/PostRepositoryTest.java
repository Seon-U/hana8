...
class PostRepositoryTest extends BaseRepositoryTest {
	...
	// 	assertThat(page1.getTotalPages()).isEqualTo(repository.count() / 10 + 1);
	...
	@Test
	void titleLikeTest() {
		List<Post> posts = repository.findByTitleStartingWith("Title1");
		System.out.println("posts = " + posts);
		posts.stream().map(Post::getTitle).forEach(System.out::println);
		assertThat(posts).isNotEmpty()
			.allSatisfy(p ->
				assertThat(p.getTitle()).contains("Title1")
			);
	}
